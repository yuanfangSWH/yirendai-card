...
export default {
  ...
  methods: {
    ...
    loadingData(id) {
      ...
      this.api.apiPost(
        ...
        (status, judge, res) => {
          if (status) {
            //信用卡标签id
            this.idData = res.data.bank_label_id;
            //信用卡标签名称
            this.goodsData = res.data.bank_label_name;
            //信用卡标签排序
            this.rulesData = res.data.bank_label_sort;
          } else {
            this.$message({
              showClose: true,
              message: res.message,
    ...
    submit() {
      var data = {
        //信用卡标签id
        bank_label_id: this.idData,
        //信用卡标签名称
        bank_label_name: this.goodsData,
        //信用卡标签排序
        bank_label_sort: this.rulesData
      };
      this.api.apiPost(
        "/lable/saveedit",
        "post",
