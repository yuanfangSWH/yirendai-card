<template>
  <div class="layer">
    ...
    <div class="black-div" v-if="exitBounced">
      <div class="bounced">
        <div class="b-bt">
          <div class="b-wz1">信用卡下架</div>
        </div>
        <div class="b-nr">
          <dl>确定要下架【{{xuanzdshuj.bank_card_name}}】吗？</dl>
          <div class="b-ann">
    ...
    <div class="black-div" v-if="exitBounced2">
      <div class="bounced">
        <div class="b-bt">
          <div class="b-wz1">信用卡上架</div>
        </div>
        <div class="b-nr">
          <dl>确定要上架【{{xuanzdshuj.bank_card_name}}】吗？</dl>
          <div class="b-ann">
    ...
    <div class="black-div" v-if="exitBounced3">
      <div class="bounced">
        <div class="b-bt">
          <div class="b-wz1">信用卡删除</div>
        </div>
        <div class="b-nr">
          <dl>确定要删除【{{xuanzdshuj.bank_card_name}}】吗？</dl>
          <div class="b-ann">
    ...
    <div class="list-div">
      ...
      <div class="l-nr-a" v-show="state==1">
        <div class="title">
          <div class="li">编号</div>
          <div class="li">所属银行</div>
          <div class="li">卡种名称</div>
          <div class="li">卡片素材</div>
          <div class="li">权益描述</div>
          <div class="li">标签内容</div>
        ...
        <div class="title" v-for="(item,key) in cardData1" :key="key">
          ...
          <div class="li">{{item.bank.bank_name}}</div>
          <div class="li">{{item.bank_card_name}}</div>
          <div class="li">
            <img :src="item.bank_card_img" />
          </div>
          <div class="li">{{item.bank_card_desc}}</div>
          <div class="li">{{item.bank_card_tab}}</div>
          <div class="li">{{item.bank_card_time}}</div>
          <div class="li">{{item.bank_card_status}}</div>
          <div class="li">{{item.bank_card_sort}}</div>
          <div class="li">
      ...
      <div class="l-nr-b" v-show="state==2">
        ...
        <div class="title" v-for="(item,key) in cardData2" :key="key">
          ...
          <div class="li">
            ...
            <span class="li-abq1" @click="skbj(item.bank_card_id)">编辑</span>
...
export default {
  ...
  methods: {
    ...
    loadingData() {
      ...
      this.api.apiPost(
        ...
        (status, judge, res) => {
          if (status) {
            if (this.status == 1) {
              this.cardData1 = res.data.info;
            } else {
              this.cardData2 = res.data.info;
            }
            //总数据量
    ...
    ShelvesAPI(value) {
      var data = {
        bank_card_id: this.xuanzdshuj.bank_card_id,
        bank_card_status: value
      };
      this.api.apiPost(
        "/creditcard/pullos",
        "post",
