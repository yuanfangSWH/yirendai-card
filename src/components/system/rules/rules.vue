<template>
  <div class="layer">
    ...
    <div class="black-div" v-if="exitBounced">
      <div class="bounced">
        <div class="b-bt">
          <div class="b-wz1">规则下架</div>
        </div>
        <div class="b-nr">
          <dl>确定要下架【{{xuanzdshuj.channel_gradename}}】吗？</dl>
          <div class="b-ann">
    ...
    <div class="black-div" v-if="exitBounced2">
      <div class="bounced">
        <div class="b-bt">
          <div class="b-wz1">规则上架</div>
        </div>
        <div class="b-nr">
          <dl>确定要上架【{{xuanzdshuj.channel_gradename}}】吗？</dl>
          <div class="b-ann">
    ...
    <div class="black-div" v-if="exitBounced3">
      <div class="bounced">
        <div class="b-bt">
          <div class="b-wz1">规则删除</div>
        </div>
        <div class="b-nr">
          <dl>确定要删除【{{xuanzdshuj.channel_gradename}}】吗？</dl>
          <div class="b-ann">
    ...
    <div class="search-div">
      <router-link to="/rulestj">
        <div class="tjia">添加规则</div>
      </router-link>
    </div>
    <!-- 列表 -->
    <div class="list-div">
      ...
      <div class="l-nr-a" v-show="state==1">
        <div class="title">
          <div class="li">编号</div>
          <div class="li">等级名称</div>
          <div class="li">等级</div>
          <div class="li">业绩要求</div>
          <div class="li">奖励系数</div>
          <div class="li">添加日期</div>
          <div class="li">排序</div>
          <div class="li">操作</div>
        </div>
        <div class="title" v-for="(item,key) in cardData" :key="key">
          <div class="li">{{item.channel_id}}</div>
          <div class="li">{{item.channel_gradename}}</div>
          <div class="li">{{item.channel_grade}}</div>
          <div class="li">{{item.channel_requirement}}</div>
          <div class="li">{{item.channel_reward}}</div>
          <div class="li">{{item.created_at}}</div>
          <div class="li">{{item.channel_sort}}</div>
          <div class="li">
            <span class="li-abq1" style="margin-left: 10px;" @click="shelves(1,item)">下架</span>
          </div>
        </div>
        <div class="paging">
          <el-pagination
      ...
      <div class="l-nr-b" v-show="state==2">
        ...
        <div class="title" v-for="(item,key) in cardData" :key="key">
          ...
          <div class="li">
            ...
            <span class="li-abq1" @click="skbj(item.channel_id)">编辑</span>
  ...
  methods: {
    ...
    loadingData() {
      ...
      this.api.apiPost(
        ...
        (status, judge, res) => {
          if (status) {
            this.cardData = res.data.channel;
            //总数据量
    ...
    ShelvesAPI(value) {
      var data = {
        channel_id: this.xuanzdshuj.channel_id,
