...
export default {
  ...
  //生命周期-页面初始化完成加载
  created() {
    this.loadingData(this.$route.query.id, this.$route.query.flid);
  },
  methods: {
    //加载数据
    loadingData(id, flid) {
    ...
    loadingData2(id, flid) {
      ...
      this.api.apiPost(
        ...
        (status, judge, res) => {
          if (status) {
            //bannerid
            this.idData = res.data.banner.banner_id;
            //跳转链接
            this.urlData = res.data.banner.banner_href;
            //排序
            this.numberData = res.data.banner.banner_sort;
            //图片
            this.portrait = res.data.banner.banner_img;
            //分类值
            this.xialj = String(res.data.bannertype.banner_cate_id);
          } else {
            this.$message({
              showClose: true,
              message: res.message,
    ...
    submit() {
      ...
      this.api.apiPost("/banner/saveedit", "post", data, "", (status, judge, res) => {
        if (status) {
          this.$notify({
            title: "保存成功",
            type: "success",
